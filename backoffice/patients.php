...
<div class="">
	<ul class="bg-dark-blue">
		<br/>
		<li>The Prescription Hope Pharmacy Program requires client participation between the client and Prescription Hope only.</li>
		<ul>
			<li>If the client does not participate when needed, medication orders will not be fulfilled.</li>
			<li>Proof of income documentation, among few other items, will be requested by each client as a requirement of the pharmaceutical company to ship the medications.</li>
			<li>Participation is minimal for the client.</li>
		</ul>
		<br/>
	</ul>
		...
		<table>
			...
			<?php foreach ($enrolled_patients->patients as $patient) { ?>
				<tr class="top-separator">
					...
					<?php if (!(bool) $_SESSION['annual_commission']) { ?><td><?=$_SESSION['agent_first_name']?> <?=$_SESSION['agent_middle_name']?> <?=$_SESSION['agent_last_name']?></td><?php } ?>
					<?php if (!(bool) $_SESSION['annual_commission']) { ?><td class="nobr"><?=$_SESSION['agent']?></td><?php } ?>
