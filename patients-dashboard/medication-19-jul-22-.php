														...
														<table class="table">
															...
															<?php
																...
																foreach ($rxi_data->meds as $key => $med) { 
																	...
																	if (isset($med->InAppealsProcessing) && $med->InAppealsProcessing) $intAlerts++;
																	if (isset($med->NeedPatientInfo) && $med->NeedPatientInfo) $intAlerts++;
																	if (isset($med->NeedPatientInfo) && $med->NeedProviderInfo) $intAlerts++;
																...
																<tr class="med_item <?php echo (isset($med->MedEstArrivalDate) && $med->MedEstArrivalDate!='' ) ? 'has_date' :'';?>" id="med<?=$med->MedAssistDetailID?>">
																	...
																	<td id="view<?=$med->MedAssistDetailID?>"><a href="javascript:void(0);" class="view-medi <?php if($firstMed==1){ echo 'firstmed'; }?>"><i class="fa fa-eye"></i> <span>View Details</span></a></td>
																	
																	<td>
																	<?php
																	if( isset($med->AppStatus) && $med->AppStatus!='' ){
																		if( $med->AppStatus==2 && isset($med->MedEstArrivalDate) ) { ?>
																			<?php $etadate = (isset($med->MedEstArrivalDate) && $med->MedEstArrivalDate!='') ? date('d/M', strtotime($med->MedEstArrivalDate. ' + 11 days')) : '';
																			if( $etadate!='' ){
																			...
																			<?php } else { echo '<span class="med_processing req_approved">Request Approved & Processing</span>'; }
																		}
						...
						<div class="panel panel-default panel-info Profile meddesc-box" id="meddesc<?=$med->MedAssistDetailID?>" style="display: none;">
							<div class="panel-heading">
								<span class="pull-left" data-dismiss1="alert" id="close_d_v">
									<a <?php if($med->ProviderId== -1){ echo 'style="display:none;"';}?> href="javascript:editMedication('<?=$med->MedAssistDetailID?>');"><img src="<?= $basepath?>/images/Medication/edit.png"> CHANGE DOSAGE </a>
								</span>
...
jQuery(document).ready(function(){
	...
	if(location.hash.substr(1)=='add_new_med'){
		...
		setTimeout(function(){ var medname = (urlParams.query).replace('med_name=','');
		jQuery('#DrugNameRow #DrugAppliedFor').val( decodeURI(medname.trim()) );}, 1000);
		addMedication();
	}	
});
