...
$pdf = new FPDI();
$pageCount = $pdf->setSourceFile("PH-Enrollment-Form.pdf");
...
if (isset($data['doctors']) && count($data['doctors']) > 0) {
	//- Name
	$pdf->SetXY(30, 103);
	$pdf->MultiCell(75, 6, ((isset($data['doctors'][0]['doctor_first_name']) && isset($data['doctors'][0]['doctor_last_name'])) ? $data['doctors'][0]['doctor_first_name'] . ' ' . $data['doctors'][0]['doctor_last_name'] : ''), $showCellBorder, 'C');
	//- Facility Name
	$pdf->SetXY(30, 109);
	$pdf->MultiCell(75, 6, ((isset($data['doctors'][0]['doctor_facility'])) ? $data['doctors'][0]['doctor_facility'] : ''), $showCellBorder, 'C');
	//- Address / Suite
	$pdf->SetXY(30, 115);
	$pdf->MultiCell(75, 6, ((isset($data['doctors'][0]['doctor_address'])) ? $data['doctors'][0]['doctor_address'] : '') . ((isset($data['doctors'][0]['doctor_address2']) && $data['doctors'][0]['doctor_address2'] != '') ? ', ' . $data['doctors'][0]['doctor_address2'] : ''), $showCellBorder, 'C');
	//- City / State / Zip
	$pdf->SetXY(30, 122);
	$pdf->MultiCell(75, 6, ((isset($data['doctors'][0]['doctor_city'])) ? $data['doctors'][0]['doctor_city'] : '') . ((isset($data['doctors'][0]['doctor_state'])) ? ', ' . $data['doctors'][0]['doctor_state'] : '') . ((isset($data['doctors'][0]['doctor_zip'])) ? ', ' . $data['doctors'][0]['doctor_zip'] : ''), $showCellBorder, 'C');
	//Phone
	$pdf->SetXY(22, 128);
	$pdf->MultiCell(10, 6, ((isset($data['doctors'][0]['doctor_phone'])) ? substr(preg_replace("/[^0-9]/", '', $data['doctors'][0]['doctor_phone']), 0, 3) : ''), $showCellBorder, 'C');
	//
	$pdf->SetXY(32, 128);
	$pdf->MultiCell(10, 6, ((isset($data['doctors'][0]['doctor_phone'])) ? substr(preg_replace("/[^0-9]/", '', $data['doctors'][0]['doctor_phone']), 3, 3) : ''), $showCellBorder, 'C');
	//
	$pdf->SetXY(43, 128);
	$pdf->MultiCell(14, 6, ((isset($data['doctors'][0]['doctor_phone'])) ? substr(preg_replace("/[^0-9]/", '', $data['doctors'][0]['doctor_phone']), 6, 4) : ''), $showCellBorder, 'L');
	//Fax
	$pdf->SetXY(68, 128);
	$pdf->MultiCell(10, 6, ((isset($data['doctors'][0]['doctor_fax'])) ? substr(preg_replace("/[^0-9]/", '', $data['doctors'][0]['doctor_fax']), 0, 3) : ''), $showCellBorder, 'C');
	//
	$pdf->SetXY(78, 128);
	$pdf->MultiCell(10, 6, ((isset($data['doctors'][0]['doctor_fax'])) ? substr(preg_replace("/[^0-9]/", '', $data['doctors'][0]['doctor_fax']), 3, 3) : ''), $showCellBorder, 'C');
	//
	$pdf->SetXY(89, 128);
	$pdf->MultiCell(14, 6, ((isset($data['doctors'][0]['doctor_fax'])) ? substr(preg_replace("/[^0-9]/", '', $data['doctors'][0]['doctor_fax']), 6, 4) : ''), $showCellBorder, 'L');
}
...
if (isset($data['doctors']) && count($data['doctors']) > 1) {
	...
	//- Address / Suite
	$pdf->SetXY(127, 115);
	$pdf->MultiCell(75, 6, ((isset($data['doctors'][1]['doctor_address'])) ? $data['doctors'][1]['doctor_address'] : '') . ((isset($data['doctors'][1]['doctor_address2'])) ? ', ' . $data['doctors'][1]['doctor_address2'] : ''), $showCellBorder, 'C');
	//- City / State / Zip
	$pdf->SetXY(127, 122);
...
//Salary
$pdf->SetXY(44, 192);
$pdf->MultiCell(30, 6, ((isset($data['p_income_salary'])) ? sprintf('%0.2f', $data['p_income_salary']) : ''), $showCellBorder, 'C');

//Pension
$pdf->SetXY(111, 192);
$pdf->MultiCell(30, 6, ((isset($data['p_income_pension'])) ? sprintf('%0.2f', $data['p_income_pension']) : ''), $showCellBorder, 'C');

//SS Retirement
$pdf->SetXY(177, 192);
$pdf->MultiCell(25, 6, ((isset($data['p_income_ss_retirement'])) ? sprintf('%0.2f', $data['p_income_ss_retirement']) : ''), $showCellBorder, 'C');

//Unemployment
$pdf->SetXY(44, 199);
$pdf->MultiCell(30, 6, ((isset($data['p_income_unemployment'])) ? sprintf('%0.2f', $data['p_income_unemployment']) : ''), $showCellBorder, 'C');

//Annuity
$pdf->SetXY(111, 199);
$pdf->MultiCell(30, 6, ((isset($data['p_income_annuity'])) ? sprintf('%0.2f', $data['p_income_annuity']) : ''), $showCellBorder, 'C');

//SS Disability
$pdf->SetXY(177, 199);
$pdf->MultiCell(25, 6, ((isset($data['p_income_ss_disability'])) ? sprintf('%0.2f', $data['p_income_ss_disability']) : ''), $showCellBorder, 'C');

//No Income
$pdf->SetXY(44, 216);
$pdf->MultiCell(20, 6, ((isset($data['p_income_zero']) && (bool) $data['p_income_zero']) ? 'X' : ''), $showCellBorder, 'C');

//No Tax Filing
$pdf->SetXY(140, 216);
$pdf->MultiCell(20, 6, ((isset($data['p_income_file_tax_return']) && (bool) $data['p_income_file_tax_return']) ? 'X' : ''), $showCellBorder, 'C');
...
if (isset($data['p_payment_method']) && $data['p_payment_method'] == 'cc') {
	...
	for ($i = 0; $i < 16; $i++){
		...
		$pdf->MultiCell(5, 6, $cc_no[$i], $showCellBorder, 'C');
	}
	// - CC Exp Month
...
//#1 Signature
$pdf->SetXY(194, 92);
$pdf->MultiCell(5, 5, ((isset($data['p_payment_agreement']) && (bool) $data['p_payment_agreement']) ? 'X' : ''), $showCellBorder, 'C');

//#2 Signature
$pdf->SetXY(194, 154);
$pdf->MultiCell(5, 5, ((isset($data['p_service_agreement']) && (bool) $data['p_service_agreement']) ? 'X' : ''), $showCellBorder, 'C');

//#3 Signature
$pdf->SetXY(194, 204);
$pdf->MultiCell(5, 5, ((isset($data['p_guaranty_agreement']) && (bool) $data['p_guaranty_agreement']) ? 'X' : ''), $showCellBorder, 'C');

//#4 Signature
$pdf->SetXY(37, 232);
$pdf->MultiCell(50, 6, 'Submitted Online', $showCellBorder, 'C');

//Date
$pdf->SetXY(21, 241);
$pdf->MultiCell(28, 5, date('m/d/Y'), $showCellBorder, 'C');

$pdf->Output('Prescription_Hope_Application.pdf', 'I');
