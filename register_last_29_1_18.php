...
if (isset($_SERVER["HTTP_REFERER"]) && $_SERVER["HTTP_REFERER"] != '' && strripos($_SERVER["HTTP_REFERER"], 'prescriptionhope.com') === false) {
...
$data_request = array(
	...
	'p_middle_initial'			=> array(false, 1),
	...
	'p_hear_about_1' 			=> array(false, 2),
	'p_hear_about_2' 			=> array(false, 2),
	'p_hear_about_3' 			=> array(false, 2),
...
if (!isset($_POST['bSubmit'])) {
	...
	$data['p_hear_about'] = (isset($_SESSION['my_source']) && is_array($agent_details)) ? trim(urldecode($_SESSION['my_source'])) . $agent_details['agent_full_name'] : '';
		...
		<div class="container">
			<div id='applicationForm'>
				<?php if (!$form_submitted) { ?>
						...
						jQuery().ready(function() {
							...
							jQuery("#register_form").validate({
								rules: {
									...
									p_cc_number:				{ required: jQuery("#p_payment_method_cc"), creditcardtypes: function(element) {return {visa: (jQuery('select[name=p_cc_type]').val() == "Visa"), mastercard: (jQuery('select[name=p_cc_type]').val() == "Mastercard"), amex: (jQuery('select[name=p_cc_type]').val() == "American Express"), discover: (jQuery('select[name=p_cc_type]').val() == "Discover")};}},
							...
							jQuery('input.input_zero').change(function() {
							    ...
							    jQuery(this).val((jQuery(this).val() != '') ? parseFloat(jQuery(this).val()).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2, useGrouping: false}) : '0.00');
							});
							//
					...
					<h2 class="center-alignment dblue-text no-text-transformation">Watch The Prescription Hope Process</h2>
					<div class="video-panel"><div class="video-container"><iframe src="https://player.vimeo.com/video/235383572" width="640" height="360" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe></div></div>

					<h2 class="center-alignment dblue-text no-text-transformation">Enrollment Form</h2>
					...
					<p class="left-alignment moduleSubheader" style="text-transform: none;">
					...
					<p class="left-alignment moduleSubheader">
						One form per person. Fields with asterisks (*) are required.
					</p>

					<form id="register_form" method="post" action="https://manage.prescriptionhope.com/register.php">

					<h2 class="center-alignment dblue-text no-text-transformation">Contact Information</h2>
					<br/>
					...
					<h2 class="center-alignment dblue-text no-text-transformation">Patient Information</h2>
					...
					<p class="form-row">
						<label for="p_ssn" style="padding-bottom: 30px;">
							SSN *<br>
							<a href="#" rel="Your Social Security Number is required by the pharmaceutical companies for completing the application process to begin<br class='only-desktop'> filling your medication order(s). Your personal information is always kept confidential." class="skipLeave disable-click tooltip-link">Why do you need my <br class="only-desktop">Social Security Number</a>
						</label>
						<input type="text" name="p_ssn" value="<?php echo $data['p_ssn'];?>" maxlength="11" style="position: relative; top: -35px; display: inline-block;">
					</p>
					...
					<p class="form-row">
						...
						<?php } else { ?>
							<select name="p_hear_about" id="p_hear_about" data-value="<?php echo htmlspecialchars(stripslashes($data['p_hear_about']));?>" class="w800 no-margin">
					...
					<h2 class="center-alignment dblue-text no-text-transformation">Monthly Income</h2>
					...
					<p class="form-row" style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #bbb;">
						<label for="p_income_annual_income" class="w260">Total Annual Income</label>
						$ <input type="text" name="p_income_annual_income" value="<?php printf('%0.2f', preg_replace("/[^0-9.]/", "", $data['p_income_annual_income']));?>" class="" readonly rel="You must enter your total monthly income for each of the options above that you receive."><br/>
					</p>
					...
					<h2 class="center-alignment dblue-text no-text-transformation">Medication Information</h2>
					...
					<div id="new_doctor_form">
						...
						<p id="doctor_form">
							...
							<input type="text" name="dr_address" value="" class="dr-required dr-data" rel="Some health care providers have multiple locations they work from, please provide the address for the location you visit your health care provider at."><br/><br/>
					...
					<h2 class="center-alignment dblue-text no-text-transformation">Terms And Conditions</h2>

					<br/>

					<p class="moduleSubheader terms_scroll_box justify-alignment">
						<span class="policy_subtitle">Fees:</span> If we find that we are unable to access at least one of your medication(s) during the initial enrollment process, there will be no charges to your account. If we can access one or more medication(s), the first month's administrative service fee of $<?=$_SESSION['rate']?> per eligible medication will be debited upon receipt of this form. Your monthly administrative service fee of $<?=$_SESSION['rate']?> per eligible medication will be debited on the 5th day of every month thereafter unless the 5th falls on a weekend or a holiday, in which case your debit will occur on the prior business day. You will be provided details in writing for each of your eligible medications. There are no other fees or costs for your eligible medication(s).
						<br><br>
						You agree that you are not paying for medication(s) through Prescription Hope. You understand that you are paying for Prescription Hope to order, manage, track and refill your eligible medication(s) offered through pharmaceutical company patient assistance programs. You authorize Prescription Hope and/or its agents to debit the account provided by you for all administrative service fees described in this “Fees” section. If your payment is returned unpaid by your financial institution you agree to pay any associated fees, including any Insufficient Funds fees or Chargeback fees accrued by your and/or Prescription Hope’s financial institution(s). You agree there are no refunds other than what is explained in the “Guarantee” section of this enrollment form.
						<br><br>

						Your monthly transaction will appear on your bank or credit card statement as "PRESCRIPTION HOPE".
						<br><br>

						<span class="policy_subtitle">Delivery:</span> Typically, you will receive your first supply of eligible medication(s) within 4-6 weeks. This range is an average amount of time and relies on a prompt response to the information we request from you and your health care provider(s). Each eligible medication is shipped directly from its pharmaceutical company and delivered either to your home or health care provider's office, depending on the pharmaceutical companies’ delivery guidelines.
						<br><br>

						<span class="policy_subtitle">Cancellation:</span> You understand you may cancel your account at any time by providing a signed letter of cancellation, which can take up to 30 days to process. Service fees will continue to be assessed until your eligible medication supplies deplete. Upon cancellation you agree to be financially responsible for any outstanding balances.
						<br><br>

						You agree to release Prescription Hope, its agents, employees, successors and assigns from any and all liability including legal fees and costs arising from medication(s) taken by you or arising from third persons ingesting any medication(s) which were accessed through Prescription Hope, and/or your reliance on Prescription Hope in general.
					</p>
					...
					<p>
						...
						<label for="p_payment_agreement" class="no_width big">I have read and agree with the above statements *</label>
					</p>

					<p class="moduleSubheader terms_scroll_box justify-alignment">
						<span class="policy_subtitle">Service:</span> You authorize Prescription Hope to act on your behalf and to sign applications for patient assistance programs by granting to Prescription Hope a limited power of attorney for the specific purposes of enrolling you in patient assistance programs with the applicable pharmaceutical companies and any related activities to process your enrollment. You authorize your health care providers' office(s) to discuss/release medical/HIPAA-related information to Prescription Hope relating to your application(s) for patient assistance programs that Prescription Hope is processing on your behalf.
						<br><br>

						You understand that Prescription Hope does not prescribe, ship, purchase, sell, handle, or dispense prescription medication of any kind in its efforts to process your application(s) for patient assistance programs. Prescription Hope assembles and submits your application to the pharmaceutical company but does not participate in the review process to determine which applicants are eligible. Each medication is provided by the pharmaceutical companies’ patient assistance programs at no cost to the eligible individual.
						<br><br>

						You understand that it is each individual pharmaceutical company who makes the final decision as to whether you qualify for their program(s). You understand Prescription Hope reserves the right to change its services to you at any time, with or without notice. You understand each pharmaceutical company sets its own eligibility guidelines and determines which products are included in their programs. Eligible medication availability is subject to change at any time, with or without notice.
						<br><br>

						You agree that you may be contacted via telephone, cellular phone, text message or email through all numbers/addresses provided to us and authorize receipt of pre-recorded/artificial voice messages and/or use of an automated dialing service by Prescription Hope or its affiliates.
					<p>
					...
					<p>
						...
						<label for="p_service_agreement" class="no_width big">I have read and agree with the above statements *</label>
					</p>

					<p class="moduleSubheader terms_scroll_box justify-alignment">
						<span class="policy_subtitle">Guarantee:</span> If you do not receive a medication because you were determined to be ineligible by the pharmaceutical company and you have a letter of denial from them, Prescription Hope will gladly refund its monthly administrative service fee(s) for the medication(s) determined to be ineligible only after Prescription Hope has exhausted all avenues of appeal with the pharmaceutical company. All Prescription Hope requires is a copy of the denial letter that was sent to you from the pharmaceutical company explaining why you are ineligible.

						<br/><br/>

						<span class="policy_subtitle">Privacy:</span> We value our patients and make extreme efforts to protect the privacy of your personal information. Your information is used for order fulfillment-related purposes only. All of your information including HIPAA-related information will never be disclosed to any third party other than for order fulfillment-related purposes. All information given to Prescription Hope, Inc., its agents, employees, successors, and assigns (collectively, "Prescription Hope") will be kept confidential and protected in the strictest manner.

						<br/><br/>

						<span class="policy_subtitle">Eligibility:</span> If you are experiencing hardship in affording your medication and/or you currently do not have coverage that reimburses or pays for your prescription medications you may be eligible. You agree that the information provided on this form is complete and accurate. If you determine the information was not correct at the time you provided it to Prescription Hope, or if the information was accurate but is no longer accurate, you will immediately notify Prescription Hope in writing by providing the correct information.
					</p>
					...
					<p>
						...
						<label for="p_guaranty_agreement" class="no_width big">I have read and agree with the above statements *</label>
					</p>
					...
					<?php if (!(isset($agent_details['use_payment']) && (bool) $agent_details['use_payment'])) { ?>
						...
						<p class="center-alignment moduleSubheader" style="text-transform: none;">
							Rest assured, we will not charge your card until we verify we can access one or more of your medications.
						</p>

						<h2 class="center-alignment dblue-text no-text-transformation">Payment Information</h2>
						...
						<div class="pull-left">
							<p>
								<input type="radio" id="p_payment_method_cc" name="p_payment_method" value="cc" checked="checked" <?php echo (($data['p_payment_method'] != '') ? 'preload="' . $data['p_payment_method'] . '"' : ''); ?>> <label for="p_payment_method_cc" class='no_width'> &nbsp;Visa, Mastercard, American Express and Discover</label><br/><label for="p_payment_method_cc" class='no_width' style='padding-left: 21px;'>Debit/Credit Cards are accepted.</label>
								<br clear="all"/>
								<img src="images/cc_visa.png" border="0" height="43" class="payment_images_" style='padding-left: 21px;' /> &nbsp;<img src="images/cc_mastercard.png" height="43" border="0" /> &nbsp;<img src="images/cc_amex.png" height="43" border="0" /> &nbsp;<img src="images/cc_discover.png" height="43" border="0" />
								<br/><br/>
						...
						<p id="payment_cc" class="">
							...
							<label for="p_cc_cvv" class="payment_method_field">CVV Security Code (<a href="#" rel="images/cvv4.jpg" class="skipLeave disable-click">?</a>)*</label>
							<input type="text" name="p_cc_cvv" value="<?php echo $data['p_cc_cvv'];?>" maxlength="4"><br/>
						</p>
						...
						<p class="moduleSubheader justify-alignment">
							<strong>
					...
					<p class="center-alignment">
						<br/>
						<input type="submit" name="bSubmit" id="bSubmit" value="Submit" class="small-button-orange" style="width: 100%;">
					</p>
				...
				<?php } else { ?>
					...
					<h2 class="center-alignment dblue-text no-text-transformation">Prescription Hope Enrollment<br/><?php echo (($response_success) ? 'Confirmation' : 'Error'); ?></h2>
		...
		<div>
		    <div class="leavePopupContent">
		    	...
		    	<div class="leavePopupContentMain">
					...
					<p class="center-alignment no-margin-bottom">
						<textarea name="leave_reason" class="no-margin"></textarea>
					</p>
			    </div>
		    	<br>

				<div class="center-alignment" id="leaveButtons2">
					<input type="submit" name="bSubmitLeavePage" id="btLeaveSubmit" value="Submit" class="popup-button-big">
					<br>
		...
		<div>
		    <div class="leavePopupContent">
			    <div class="leavePopupContentMain">
			    	...
			    	<p class="center-alignment no-margin-bottom" style="font-size: 12px;">Press OK to Continue</p>
		    	</div>
		    	<br>
				<div class="center-alignment no-margin-bottom" id="btReminderOK">
	...
	<div class="text-container">
		<div>
		    <div class="leavePopupContent">
		    	...
		    	<br/>
				<div class="center-alignment" id="submitButtons">
...
<script type="text/javascript">
	...
	jQuery().ready(function() {
		if (jQuery('#p_hear_about').is("select")) {
