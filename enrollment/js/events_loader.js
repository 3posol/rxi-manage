jQuery().ready(function() {
	//browser detection
	jQuery('body').addClass((/iphone/i.test(navigator.userAgent)) ? 'iphone' : '');
	...
	*/
	...
	jQuery('#fmLogin').submit(function (e) {
		...
		jQuery('input').each(function() {
			if (jQuery(this).val() == '') {
				...
				jQuery(this).addClass('field-error-only');
			} else {
				jQuery('label[for="' + jQuery(this).attr('id') + '"]').removeClass('error');
				jQuery(this).removeClass('field-error-only');
			}
		});
	...
	jQuery('#fmForgotPassword').submit(function (e) {
		...
		if (!valid_form) {
			...
			jQuery('#fmMsg').html('All fields are mandatory. Please fill out all fields.<br/><br/>')
			jQuery('#fmMsg').addClass('error');
		} else {
			jQuery('#fmMsg').html('');
			jQuery('#fmMsg').removeClass('error');
    ...
    */
	...
	if (typeof google !== 'undefined') {
		google.maps.event.addDomListener(window, 'load', initializeGoogleGeoLocation);
	}

	//force patient & doctor address to fill only from Google search result
	/*
	var patient_new_address_selected = false;
	var doctor_new_address_selected = false;
	jQuery('input[name="p_address"],input[name="dr_address"]').focus(function (e) {
		//patient
		if (jQuery(this).attr('name') == 'p_address') {
			new_patient_address_selected = false;
		}

		//doctor
		if (jQuery(this).attr('name') == 'dr_address') {
			doctor_new_address_selected = false;
		}

		jQuery(this).attr('pre-search-value', jQuery(this).val());
	});
	jQuery('input[name="p_address"],input[name="dr_address"]').change(function (e) {
		//patient
		if (jQuery(this).attr('name') == 'p_address') {
			if (!new_patient_address_selected) {
				//address not selected from google
				jQuery(this).val(jQuery(this).attr('pre-search-value'));
			}
			new_patient_address_selected = false;
		}
		...
		//doctor
		if (jQuery(this).attr('name') == 'dr_address') {
			if (!doctor_new_address_selected) {
				//address not selected from google
				jQuery(this).val(jQuery(this).attr('pre-search-value'));
			}
			doctor_new_address_selected = false;
	...
	*/
	...
	jQuery('input:not([type=checkbox]):not([type=radio]), select').focus(function (e) {
		if (jQuery(this).hasClass("error")) {
			jQuery(this).removeClass("error");
			jQuery(this).addClass("field-error-only");
			jQuery('label.error[for="' + jQuery(this).attr('id') + '"]').remove();
			jQuery('label.error[for="' + jQuery(this).attr('name') + '"]').remove();
