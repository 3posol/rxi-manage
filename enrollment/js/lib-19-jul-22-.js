...
function AddNewDoctorForm (e) {
	...
	liCount = 1; ulCount = 1; rowNumber = 3 ; 
    var is_empty = false;
...
function ReorderMedSpecifiers(){
    ...
    var i = 1;
    jQuery('.medication-row .medication-doctor-field .doctors_dropdown').each(function(){
...
function showTooltipBox (e) {
	if (jQuery(this).data("tooltip") != "") {
		...
		if(jQuery(this).text() == 'hidetext'){ console.log(jQuery(this).parents('.safe-enroll').position());
...
function showMedicationList(){
    ...
    if(cur_med_id>0 && jQuery('#tr'+cur_med_id).length>0){
        ...
        if(cur_med_name!=''){
            ...
            jQuery('#med-list-box .total_str, #med_info .total_str').html('$'+(jQuery('.m-list').length)*med_price+'.00');
	...
	jQuery('.doctors_dropdown').each(function(index, elem){ 
        ...
        if(medsName != '') {
            isValid = true;
              ...
              $("#medication_form .medication-row").removeClass("in active"); //Remove active class from all the div and add to cy=urrent one 
              $("#mb"+parentElnumber[0]).addClass('in active');
