...
foreach ($files as $file) {
	...
	if ($file == 'ok-2015-09-23-0000-535tj6c3rmdqlfclvdamo7cvq4') {
		$data = array();
		...
		if (isset($data['p_first_name']) && $data['p_first_name'] != '' && isset($data['p_last_name']) && $data['p_last_name'] != '' && isset($data['p_phone']) && $data['p_phone'] != '' && isset($data['p_dob']) && $data['p_dob'] != '' && isset($data['p_ssn']) && $data['p_ssn'] != '') {
			...
			curl_setopt_array($cu, array(
				CURLOPT_URL => "http://64.233.245.241:43443/register_failed_application2.php",
				CURLOPT_POST => 1,
...
var_dump($delete_file);
		//delete the file
		if ($delete_file) {
		    $rs = rename($failed_apps_dir . '/'. $file,$failed_apps_dir . '/ok-' . date('Y-m-d-') . $file);
var_dump($rs);
