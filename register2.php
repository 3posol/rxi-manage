...
if (!isset($_GET['source']) && isset($_SESSION['my_source'])) {
	if (!isset($_SESSION['register_data']['p_hear_about']) || $_SESSION['register_data']['p_hear_about'] == '') {
		header("Location: https://manage.prescriptionhope.com/register2.php?source=" . $_SESSION['my_source']);
		die();
	}
}
		...
		<div class="container">
			<div id='applicationForm'>
				<?php if (!$form_submitted) { ?>
					...
					<form id="register_form" method="post" action="https://manage.prescriptionhope.com/register2.php">
					...
					<p class="moduleSubheader terms_scroll_box">
						...
						medication(s), the first month's administrative service fee of $<?=$_SESSION['rate']?> per
						...
						service fee of $<?=$_SESSION['rate']?> per medication will be debited on the 5th day of
						...
						to 30 days to process. Service fees will continue to be assessed until
						medication supplies deplete. Upon termination you agree to be financially
					...
					<p class="moduleSubheader terms_scroll_box">
						...
						it is up to each applicable medication manufacturer to make the eligibility
						determination. Each medication manufacturer independently sets its own
					...
					<p class="moduleSubheader terms_scroll_box">
						...
						denial letter sent to you from the applicable medication manufacturer
					...
					<?php if (!(isset($agent_details['use_payment']) && (bool) $agent_details['use_payment'])) { ?>
						...
						<p class="moduleSubheader">
							<strong>
							By checking this box, I acknowledge that I have read and agree to the terms and conditions of Prescription Hope, including the fee policy, service policy, privacy policy, and guarantee. I authorize Prescription Hope to charge my account $<?=$_SESSION['rate']?> per month, per medication that I may qualify for. Due to the service-based nature of the Prescription Hope program, I acknowledge there are no refunds other than what is explained in the Prescription Hope guarantee. If I do not receive medication because I am determined to be ineligible for the patient assistance program by the applicable pharmaceutical manufacturer(s) and I have a letter of denial, I acknowledge Prescription Hope will refund the monthly administrative service fee(s) for the medication(s) determined to be ineligible only after Prescription Hope has exhausted all avenues of appeal. In order to receive a refund, I will provide Prescription Hope a copy of the denial letter sent from the applicable medication manufacturer explaining why I am ineligible.
							<br/><br/>
